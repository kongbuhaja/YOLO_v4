...
class CSPDarknetBlock(Layer):
    def __init__(self, unit, layer, layer_num, div=2, branch_transition=True, block_pre_transition=True, block_post_transition=True, activate='Mish', kernel_initializer=glorot, **kwargs):
        super().__init__(**kwargs)
        self.unit = unit
        self.layers = [Identity()]
        self.layer_num = layer_num
        ...
        if block_pre_transition:
            self.block_pre_transition = DarknetConv(self.unit//self.div, 1, activate=self.activate, kernel_initializer=self.kernel_initializer)
        if layer == 'Conv':
            self.layers = [DarknetConv(self.unit//2, 3, activate=self.activate, kernel_initializer=self.kernel_initializer) for _ in range(self.layer_num)]
        elif layer == 'Resnet':
        ...
        if block_post_transition:
            self.block_post_transition = DarknetConv(self.unit//self.div, 1, activate=self.activate, kernel_initializer=self.kernel_initializer)
...
class ReverseCSPDarknetBlock(Layer):
    def __init__(self, unit, layer=False, activate='Mish', kernel_initializer=glorot, **kwargs):
        ...
        self.conv1 = DarknetConv(self.unit, 1, activate=self.activate, kernel_initializer=self.kernel_initializer)
        self.conv2 = DarknetConv(self.unit, 3, activate=self.activate, kernel_initializer=self.kernel_initializer)
        self.conv3 = DarknetConv(self.unit, 1, activate=self.activate, kernel_initializer=self.kernel_initializer)
        if layer == 'SPP':
            self.layer = SPP(self.unit, activate=self.activate, kernel_initializer=self.kernel_initializer)
        self.block_transition = DarknetConv(self.unit, 3, activate=self.activate, kernel_initializer=self.kernel_initializer)
        self.concat = Concatenate()
...
class CSPDarknetUpsampleBlock(Layer):
    def __init__(self, unit, activate='Mish', kernel_initializer=glorot, **kwargs):
        ...
        self.reverse_darknet_block = ReverseCSPDarknetBlock(unit, activate=self.activate, kernel_initializer=self.kernel_initializer)
...
class CSPDarknetDownsampleBlock(Layer):
    def __init__(self, unit, activate='LeakyReLU', kernel_initializer=glorot, **kwargs):
        ...
        self.reverse_darket_block = ReverseCSPDarknetBlock(self.unit, activate=self.activate, kernel_initializer=self.kernel_initializer)
