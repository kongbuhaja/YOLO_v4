...
@tf.function
def resize_padding(image, labels, out_size, random=False, mode='CONSTANT', constant_values=0):
...
@tf.function
def random_augmentation(image, labels, image_size):
    bboxes = labels[..., :4]
    ...
    for augmentation_method, prob_threshold in geometric_methods + kernel_methods + color_methods:
        image, bboxes = randomly_apply(augmentation_method, image, bboxes, image_size, prob_threshold)
    labels = tf.concat([bboxes, labels[..., 4:]], -1)

    return minmax(image, labels)

@tf.function
def randomly_apply(method, image, bboxes, image_size, prob_threshold=0.5):
    if tf.random.uniform(()) > prob_threshold:
        return method(image, bboxes, image_size)
    return image, bboxes

@tf.function
def random_scale(image, bboxes, image_size, lower=0.7, upper=1.0):
    size = tf.cast(tf.shape(image)[-2:-4:-1], tf.float32)
...
@tf.function
def random_crop(image, bboxes, image_size):
    ...
    if tf.reduce_any(bbox_LT >= bbox_RB):
        ...
        crop_LT = tf.cast(tf.random.uniform((2,)) * bbox_LT, tf.int32)
        crop_RB = tf.cast(tf.random.uniform((2,)) * (size - bbox_RB) + bbox_RB, tf.int32)
...
@tf.function
def random_flip_horizontally(image, bboxes, image_size):
...
@tf.function
def random_rotate90(image, bboxes, image_size):
...
@tf.function
def random_gaussian_blur(image, bboxes, image_size, ksize=3, sigma=1):
...
@tf.function
def random_saturation(image, bboxes, image_size, lower=0.5, upper=1.5):
    return tf.image.random_saturation(image, lower, upper), bboxes

@tf.function
def random_hue(image, bboxes, image_size, max_delta=0.08):
    return tf.image.random_hue(image, max_delta), bboxes

@tf.function
def random_contrast(image, bboxes, image_size, lower=0.5, upper=1.5):
    return tf.image.random_contrast(image, lower, upper), bboxes

@tf.function
def random_brigthness(image, bboxes, image_size, max_delta=0.12):
    return tf.image.random_brightness(image, max_delta), bboxes
...
# @tf.function
# def resize_padding(image, labels, out_size, mode='CONSTANT', constant_values=0):
#     size = tf.cast(tf.shape(image)[-2:-4:-1], tf.float32)
#     ratio = out_size/tf.reduce_max(size)
#     new_size = tf.cast(ratio * size, tf.int32)
#     pad_size = out_size - tf.cast(new_size, tf.float32)
...
#     return padded_image, resized_labels, tf.cast(pad_LT, tf.float32)